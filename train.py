...
if 1:
    ...
    D_zero        = EasyDict(func_name='training.networks_stylegan.D_basic_zero')               # Options for discriminator network.
    D_one         = EasyDict(func_name='training.networks_stylegan.D_basic_one')
    G_opt         = EasyDict(beta1=0.0, beta2=0.99, epsilon=1e-8)                          # Options for generator optimizer.
    D_zero_opt    = EasyDict(beta1=0.0, beta2=0.99, epsilon=1e-8)                          # Options for discriminator optimizer.
    D_one_opt     = EasyDict(beta1=0.0, beta2=0.99, epsilon=1e-8)
    G_loss        = EasyDict(func_name='training.loss.G_logistic_nonsaturating')           # Options for generator loss.
    D_zero_loss   = EasyDict(func_name='training.loss.D_logistic_simplegp_zero', r1_gamma=10.0) # Options for discriminator loss.
    D_one_loss    = EasyDict(func_name='training.loss.D_logistic_simplegp_one', r1_gamma=10.0)
    ...
    # Dataset.
    desc += '-ffhq256';     dataset = EasyDict(tfrecord_dir='/mnt/7/stylegan_dataset/datasets', resolution=256); train.mirror_augment = True
    ...
    #desc += '-2gpu'; submit_config.num_gpus = 2; sched.minibatch_base = 8; sched.minibatch_dict = {4: 128, 8: 128, 16: 128, 32: 64, 64: 32, 128: 16, 256: 8}
...
def main():
    ...
    kwargs.update(G_args=G, D_zero_args=D_zero, D_one_args=D_one, G_opt_args=G_opt, D_zero_opt_args=D_zero_opt, D_one_opt_args=D_one_opt, G_loss_args=G_loss, D_zero_loss_args=D_zero_loss, D_one_loss_args=D_one_loss)
